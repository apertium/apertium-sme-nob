...
translational analyses.\footnote{The untrimmed source files weigh in at
  about 3.5 MB, trimming this down to 2.6 MB is done by a script in
  our public repository which runs in $<10$ seconds, and is general
  enough that other lexc-based language pairs can easily use it.}
...
transfer lexicon have at least one alternative translation. This CG
particularly depends on valency and semantic sets (e.g. selecting the
\textbf{class} instead of \textbf{hill} translation if we see a context
word related to the EDUCATION set).
...
adjectives and 14000 common nouns in the transfer lexicon.
...
We also add special tags used only as a signal to transfer, which are
removed before generation. The causative derivation of a form gets a
tag which makes transfer form a periphrastic `let'-construction, but
we also add the same tag to all forms if the root itself is a
lexicalised causative. Verbs are also tagged according to the most
likely animacy of the agent, for pro-drop insertion.
...
\item Interchunk 1, 26 rules: simple anaphora resolution (based onmost
...
break the causality chain of the sentence, as in (\ex{1}):

% NB dieđihuvvo
%\item 
\enumsentence{{... die\dj{}ihuvvo ahte du\v{s}\v{s}e sullii 1/3 skuvllageatnegahtton ohppiin bohte oahpahussii}\\~\\
\shortexnt{7}
{... meddeles & han & at & bare & omtrent & 1/3 & skuvllageatnegahtton}
{... is.informed & he & that & only & approximately & 1/3 & skuvllageatnegahtton}
{}
\shortexnt{7}
{med & en & l{\ae}rer & kom & \textbf{de} & til & undervisning}
{with & a & teacher & came & \textbf{they} & to class}
{}
}
