...
morphological and syntactic tags (further detail on structural
transfer in Section \ref{sec:structural-transfer}).
...
written to be similar but contain factual mistakes), and the user had
to use the MT output as a guide to pick which paraphrase corresponded
with the original \sme{} sentence. Example (\ex{1}) shows the test for
one of the sentences (English glosses were of course not shown), where
the user could pick one of a., b. or c., b. being correct.

\enumsentence{
  Original: Muhto eat diehtán maid mii čáliimet (`But we didn't know what we wrote')\\
...
\enumsentence{
  Original: Goappaš riikkain lea nammaduvvon hálddahuslaš gulahallanolmmoš (`There is appointed an administrative contact from both countries')\\
  ...
  Hvor kommer kontaktpersonene fra? (`Where do the contact persons come from?')
}
...
nouns\footnote{Nouns were of the same length (+- 3 characters), pulled
  ...
  public repository (subdirectory \texttt{gisting-eval/generated}) and should be
