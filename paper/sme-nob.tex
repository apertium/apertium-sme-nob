...
complex, a purely lttoolbox-based analyser would be difficult to
maintain.
...
Without derivations, on the other hand, `geafivuohta' would either be lexicalised and
...
The results are shown in table \ref{table:wer}\footnote{Our post-edits are
...
of text were shown one at a time. The results are shown in table (\ref{eval}).
...
{If he not had to the possible parents travels, ...}}
...
give problems when they form part of complex NP's.
...
by the rules that are in place \footnote{an HMM after CG would not help
